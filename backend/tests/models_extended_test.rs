...
fn test_news_source_display() {
    ...
    assert_eq!(
        NewsSource::Other("Custom".to_string()).to_string(),
        "Custom"
    );
}
...
fn test_news_source_from_string() {
    ...
    assert!(matches!(
        NewsSource::from_string("RealGM"),
        NewsSource::RealGM
    ));
    assert!(matches!(
        NewsSource::from_string("HoopsHype"),
        NewsSource::HoopsHype
    ));
...
fn test_determine_category_method() {
    ...
    // 大文字小文字のテスト
    assert_eq!(
        NewsItem::determine_category("The Lakers TRADE LeBron", None),
        "Trade"
    );
    assert_eq!(
        NewsItem::determine_category("player SIGNING announced", None),
        "Signing"
    );

    // 複数のキーワードが含まれる場合（最初にマッチしたもの）
    assert_eq!(
        NewsItem::determine_category("Trade talks about signing new player", None),
        "Trade"
    );

    // 部分文字列としてのマッチ
    assert_eq!(
        NewsItem::determine_category("Blockbuster traded announced", None),
        "Trade"
    );
    assert_eq!(
        NewsItem::determine_category("Multi-year contract extension", None),
        "Signing"
    );

    // 特殊文字を含む場合
    assert_eq!(
        NewsItem::determine_category("BREAKING: Lakers acquire star", None),
        "Trade"
    );
    assert_eq!(
        NewsItem::determine_category("UPDATE: Waived by team", None),
        "Signing"
    ); // waived は signing_keywords に含まれる
    ...
    // キーワードがない場合
    assert_eq!(
        NewsItem::determine_category("NBA game highlights from last night", None),
        "Other"
    );

    // descriptionも考慮される場合
    assert_eq!(
        NewsItem::determine_category("Lakers News", Some("Team trades player")),
        "Trade"
    );
}
...
fn test_rss_feeds_constant() {
    ...
    // 主要なソースが含まれていることを確認
    let has_espn = RSS_FEEDS
        .iter()
        .any(|(_, source)| matches!(source, NewsSource::ESPN));
