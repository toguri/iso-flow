...
# S3 Bucket Public Access Block (Frontend)
...
resource "aws_s3_bucket" "mwaa_dags" {
  bucket = "${local.name}-mwaa-dags"

  tags = merge(var.tags, {
    Name = "${local.name}-mwaa-dags"
    Type = "MWAA"
  })
}
...
# S3 Bucket Public Access Block (MWAA)
...
resource "aws_cloudfront_origin_access_identity" "frontend" {
  comment = "${local.name} frontend OAI"
}
